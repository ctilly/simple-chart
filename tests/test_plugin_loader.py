...
def test_load_plugin_directory_registers_indicator(tmp_path: Path) -> None:
    ...
    plugin_file.write_text(
        dedent(
            ...
            from simplechart.api import Indicator, OHLCVSeries, register_indicator


            class CustomPluginIndicator(Indicator):

                def name(self) -> str:
                    return "custom_plugin_indicator"

                def label(self) -> str:
                    return "Custom Plugin Indicator"
            ...
            register_indicator(CustomPluginIndicator)
            """
        )
    )
    ...
    indicator = all_indicators()["custom_plugin_indicator"]()
    assert indicator.label() == "Custom Plugin Indicator"
