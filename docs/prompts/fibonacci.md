# Task: Build the "Fibonacci Retracement" indicator for SimpleChart

## Start here (do not skip)
You are adding a new indicator to the SimpleChart repo. Before doing anything else:
1. Read `AGENTS.md` in full.
2. Read `docs/skills/interactive-indicator.md` in full.
This indicator is INTERACTIVE (click-to-anchor, live mouse tracking, commit
click, drag, Configure dialog, persistent drawing). The interactive skill file
governs the entire process — its Phase Gates, Spec template, Non-Negotiable
Boundaries, and the "Fibonacci Retracement Acceptance Bar" are binding.
...
Do not write any indicator code before the boundary/file plan gate is approved.
...
## What the indicator must do (requirements)
...
indicator-only change is insufficient, list files, state behavioral risk,
define focused tests) and wait for approval.
- Register the indicator (and a store handler, if persistence is needed) from
inside the package. Import indicator-facing API only from `simplechart.api`.
...
3. Indicator spec using the skill file's spec template.
...
`python -m compileall app chart indicators simplechart tests` run clean.